package dev.neovoxel.neobot.metrics;
...
public class Metrics {
     ...
     */
    public Metrics(NeoBot plugin, int serviceId) {
        this.plugin = plugin;
        DatabaseTable table = plugin.getStorageProvider().getStorage().table("neobot_metrics");
        table.create()
                .column("key", "TEXT", "PRIMARY KEY")
                .column("value", "TEXT")
                .execute();
        List<Row> serverUuid = table.select("value").where("key", "serverUuid").execute().map();
        String serverUUID;
        if (serverUuid.isEmpty()) {
            serverUUID = UUID.randomUUID().toString();
            table.insert().column("key", "serverUuid").column("value", serverUUID).execute();
        } else {
            serverUUID = serverUuid.get(0).getString("value");
        }
        metricsBase =
                new MetricsBase(
                        "bukkit",
                        ...
                        plugin::submitAsync,
                        () -> true,
                        (message, error) -> this.plugin.getNeoLogger().warn(message + error.toString()),
                        (message) -> this.plugin.getNeoLogger().info(message),
                        false,
                        false,
                        false,
                        false);
    ...
    private void appendPlatformData(JsonObjectBuilder builder) {
        ...
        builder.appendField("onlineMode", plugin.getOnlineMode() ? 1 : 0);
        builder.appendField("bukkitVersion", plugin.getMinecraftVersion());
        builder.appendField("bukkitName", plugin.getBrand());
    ...
    private int getPlayerAmount() {
        return plugin.getOnlinePlayers().length;
    }
